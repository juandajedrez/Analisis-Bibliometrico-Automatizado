...
def build_citation_graph(threshold=0.4):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                sim = float(sim_matrix[i, j])  # 👈 asegurar float
                if sim > threshold:
                    G.add_edge(keys[i], keys[j], weight=sim)

    # 5️⃣ Validar pesos de aristas y eliminar problemáticas
